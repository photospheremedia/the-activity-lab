...
const LEGAL_LINKS: FooterLink[] = [
  { label: 'Privacy Policy', to: '/privacy', route: true },
  { label: 'Terms of Service', to: '/terms', route: true },
...
function FooterLinkItem({
  ...
  if ('action' in link && link.action === 'cookies') {
    return (
      <button type="button" onClick={onCookies} className={`text-start ${className}`}>
        {t(link.label)}
      </button>
    )
  }
    ...
    <>
      <footer className="border-t border-white/10 bg-forest-950">
        <div className="mx-auto max-w-6xl px-6 py-16">
          ...
          <div className="mt-12 flex flex-wrap items-center gap-x-6 gap-y-3 border-t border-white/10 pt-8 text-xs text-sand-100/55">
            <span className="inline-flex items-center gap-2">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-400" />
              {t('Member · Adventure Travel Trade Association')}
            </span>
            <span>{t('IFMGA-certified guides')}</span>
            <span>{t('Fully licensed & insured')}</span>
          </div>

          <div className="mt-8 flex flex-col gap-4 text-xs text-sand-100/45 md:flex-row md:items-center md:justify-between">
            <p>
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-2">
              {LEGAL_LINKS.map((link) => (
                <FooterLinkItem
                  ...
                  link={link}
                  onCookies={() => setCookiesOpen(true)}
                  className="transition-colors hover:text-sand-100/80"
                />
              ))}
            </div>
            <p>{t('Made with care in Oaxaca, Mexico')}</p>
          </div>
        </div>
      </footer>

      <CookieModal open={cookiesOpen} onClose={() => setCookiesOpen(false)} />
    </>
  )
}
